...
def test_run_select_query(db_tool: SQLDatabase):
    ...
    result = db_tool.run("SELECT name, age FROM users WHERE id = 1;")
...
def test_run_insert_query(db_tool: SQLDatabase):
    ...
    result = db_tool.run(insert_query)
    assert "1 rows affected" in result

    select_result = db_tool.run("SELECT * FROM users WHERE id = 3;")
    assert len(select_result) == 1
    assert select_result[0]["name"] == "Charlie"


def test_run_query_error(db_tool: SQLDatabase):
    """Tests that a malformed query returns an error message."""
    result = db_tool.run("SELECT * FROM non_existent_table;")
